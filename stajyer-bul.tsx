...
function StajyerBulPage() {
  ...
  // Teklif Gönderme Fonksiyonu (İşletmeler için hızlı başvuru/teklif)
  const handleSendOffer = async (candidate: any) => {
    if (!userProfile || userProfile.role !== "isveren") {
      alert("Staj teklifi gönderebilmek için işveren hesabıyla giriş yapmış olmalısınız.");
      return;
    }

    const message = prompt(`${candidate.full_name} adlı adaya göndermek istediğiniz staj teklifini veya mesajınızı yazın:`);
    if (!message) return;

    // Veritabanına teklif/mesaj kaydı atılabilir veya bildirim tablosu varsa eklenebilir
    alert(`Teklifiniz ${candidate.full_name} adlı adaya başarıyla iletildi! Adayın iletişim bilgileri: ${candidate.phone || candidate.email}`);
  };
  ...
  const filteredCandidates = candidates.filter((item) => {
    ...
    const matchesTerm = selectedTerm === "tumu" || (item.internship_term || "").toLowerCase().includes(selectedTerm.toLowerCase());
    const matchesLocation = !selectedLocation || (item.city || item.location || "").toLowerCase().includes(selectedLocation.toLowerCase());
  ...
  const isEmployer = userProfile && userProfile.role === "isveren";
    ...
    <div className="flex min-h-screen flex-col bg-background text-foreground">
      ...
      <main className="flex-1 container-x py-10">
        ...
        <div className="mb-8">
          ...
          <p className="text-muted-foreground mt-1">İşletmeniz için uygun adayları filtreleyin ve tek tıkla staj teklifi gönderin.</p>
        </div>
        ...
        {!isEmployer && (
          <div className="mb-8 p-4 rounded-xl border border-amber-500/20 bg-amber-500/10 text-amber-600 flex items-center gap-3">
            ...
            <p className="text-xs sm:text-sm font-medium">
              Not: Adaylara doğrudan staj teklifi gönderebilmek için işveren hesabıyla giriş yapmanız gerekir.
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8 p-5 rounded-2xl border border-border bg-card shadow-sm">
          <div>
            <label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Arama (İsim / Yetenek)</label>
          ...
          <div>
            <label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Bölüm / Alan</label>
            <input
              type="text"
          ...
          <div>
            <label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Şehir / Konum</label>
            <input
              type="text"
          ...
          <div>
            <label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Staj Vadesi</label>
            <select
              value={selectedTerm}
        ...
        {loading ? (
          ...
          <div className="text-center py-20 rounded-2xl border border-border bg-card p-8">
            ...
            <p className="text-sm text-muted-foreground mt-1">Seçtiğiniz kriterlere uygun aktif staj arayan aday bulunmuyor.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredCandidates.map((candidate) => (
              <div key={candidate.id} className="flex flex-col justify-between rounded-2xl border border-border bg-card p-6 shadow-sm">
                <div>
                  <div className="flex items-start justify-between gap-3 mb-4">
                    ...
                    <div className="size-14 rounded-2xl bg-primary/10 text-primary flex items-center justify-center font-bold text-xl shrink-0 overflow-hidden border border-primary/20 shadow-inner">
                      {candidate.avatar_url ? (
                        <img src={candidate.avatar_url} alt={candidate.full_name} className="size-full object-cover" />
                      ) : (
                  ...
                  <div className="space-y-1 mt-2">
                    <p className="text-sm font-semibold text-primary flex items-center gap-1.5">
                      <BookOpen className="size-4 shrink-0" /> {candidate.department || "Bölüm belirtilmemiş"}
                    </p>
                    <p className="text-xs text-muted-foreground flex items-center gap-1.5">
                      <School className="size-4 shrink-0" /> {candidate.school || "Okul belirtilmemiş"} {candidate.grade ? `(${candidate.grade})` : ""}
                    </p>
                  </div>
                  ...
                  <div className="space-y-1.5 mt-4 text-xs text-muted-foreground border-t border-border pt-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span>Konum: <strong className="text-foreground">{candidate.city || candidate.location || "Belirtilmemiş"}</strong></span>
                    </div>
                  </div>
                ...
                {/* İşverenler İçin Teklif Gönder Butonu */}
