...
import { slugify } from "./blog-helpers.js";
const cookieName = "admin_session";
const json = (body, status = 200, headers = {}) => Response.json(body, { status, headers: { "Cache-Control": "no-store", ...headers } });
function signature(payload) {
    const secret = process.env["ADMIN_SESSION_SECRET"];
    if (!secret)
        throw new Error("Yönetici oturum ayarları eksik.");
    return createHmac("sha256", secret).update(payload).digest("hex");
}
export function authenticated(cookie) {
    const token = cookie
        ?.split(";")
        .map((p) => p.trim())
    ...
    if (!payload || !signed || !/^\d+$/.test(payload) || !/^[a-f0-9]{64}$/.test(signed))
        return false;
...
function serviceClient() {
    ...
    if (!url || !key)
        throw new Error("Sunucu veritabanı ayarları eksik.");
...
function blogPayload(body) {
    if (![body.title, body.excerpt, body.content, body.category].every((value) => typeof value === "string" && value.trim()))
    ...
    const keywords = Array.isArray(body.keywords)
        ? body.keywords.filter((value) => typeof value === "string").slice(0, 20)
        : [];
    return {
        ...
        author_name: typeof body.author_name === "string" ? body.author_name.trim() : "Stajyer Bul Ekibi",
        ...
        seo_title: typeof body.seo_title === "string" && body.seo_title.trim()
            ...
            : null,
        seo_description: typeof body.seo_description === "string" && body.seo_description.trim()
...
export async function handleAdminRequest(request) {
    ...
    try {
        ...
        if (path === "/api/admin-login" && request.method === "POST") {
            ...
            if ((username !== undefined && !secureEqual(username, expectedUsername)) ||
        ...
        if (path === "/api/blog-generate" && request.method === "POST") {
            ...
            if (!key)
                return json({ error: "Yapay zeka hizmeti yapılandırılmamış. Makaleyi formdan yazabilirsiniz." }, 503);
            ...
            if (![article.title, article.excerpt, article.content].every((v) => typeof v === "string" && v.trim()))
        ...
        if (path === "/api/blog-posts") {
            ...
            if (request.method === "POST") {
                ...
                const base = slugify(typeof body.slug === "string" && body.slug.trim() ? body.slug : body.title);
                if (!base)
                    return json({ error: "Geçerli bir başlık veya slug girin." }, 400);
                for (let i = 1; i <= 50; i++) {
                    const { data, error } = await db
                        .from("blog_posts")
                        .insert({
            ...
            if (request.method === "PATCH") {
                ...
                const slug = slugify(typeof body.slug === "string" && body.slug.trim() ? body.slug : values.title);
                if (!slug)
                    return json({ error: "Geçerli bir slug girin." }, 400);
                const { data, error } = await db
                    .from("blog_posts")
                    .update({ ...values, slug })
                    .eq("id", body.id)
        ...
        return json({ error: "İşlem tamamlanamadı. Sunucu ayarlarını ve veritabanı bağlantısını kontrol edin." }, 500);
    }
}
