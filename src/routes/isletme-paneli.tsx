...
function IsletmePaneliPage() {
  ...
  const checkUserAndProfile = async () => {
    ...
    try {
      ...
      setLoadError(error instanceof Error ? error.message : "İşletme profili yüklenemedi.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchMyListings = async (employerId: string) => {
    ...
    if (error) {
      setLoadError("İlanlar yüklenemedi: " + error.message);
      return;
    }
  ...
  const handleEmployerApplication = async (e: React.FormEvent) => {
    ...
    const lowerAddress = address.toLocaleLowerCase("tr-TR").trim();
    const hasProfanity = PROFANITY_LIST.some((word) =>
      [
        ...lowerAddress.split(/[^\p{L}]+/u),
        ...companyName.toLocaleLowerCase("tr-TR").split(/[^\p{L}]+/u),
      ].includes(word.trim()),
    );
    ...
    try {
      ...
      setAppError(error instanceof Error ? error.message : "Başvuru gönderilemedi.");
    } finally {
      setSubmittingApp(false);
    }
  };
  ...
  const handleCreateListing = async (e: React.FormEvent) => {
    ...
    if (creating) return;
    if (!user || profile?.role !== "isveren" || profile?.approval_status !== "onaylandi") {
      setFormError("Onaylı işveren hesabı gerekli.");
      return;
    }
    setCreating(true);
    try {
  ...
  const handleDeleteListing = async (id: string) => {
    ...
    const { error } = await supabase
      ...
      .single();
    if (!error) {
      setMyListings((items) => items.filter((item) => item.id !== id));
    } else {
  ...
  if (loadError)
    return (
      <div className="container-x py-12">
        ...
        <button onClick={checkUserAndProfile}>Yeniden dene</button>
      </div>
    );
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen flex-col bg-background text-foreground">
        <Navbar />
        <main className="flex-1 container-x py-20 text-center text-muted-foreground">
          Paneller yükleniyor...
        </main>
        <Footer />
      </div>
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background text-foreground">
      ...
      <main className="flex-1 container-x py-10">
        ...
        {(!hasApplied || profile?.approval_status === "reddedildi") && (
          <div className="max-w-2xl mx-auto rounded-2xl border border-border bg-card p-8 shadow-sm">
            <div className="text-center mb-8">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Stajyer ilanları yayınlayabilmek için şirket bilgilerinizi eksiksiz doldurun.
              </p>
            </div>
        ...
        {hasApplied && isPending && (
          <div className="max-w-xl mx-auto text-center p-10 rounded-2xl border border-amber-500/20 bg-amber-500/5 shadow-sm">
            ...
            <p className="text-muted-foreground text-sm mt-2">
              <strong>{profile.company_name}</strong> adına yaptığınız işletme başvurusu şu anda
              yöneticilerimiz tarafından incelenmektedir. Onaylandığında tam yetkili işletme
              paneline erişebileceksiniz.
            </p>
          </div>
        )}
              ...
              <form onSubmit={handleUpdateProfile} className="space-y-4 text-sm">
                ...
                <div className="flex justify-end">
                  <button
                    type="submit"
                    disabled={creating}
                    className="rounded-xl bg-secondary px-6 py-2.5 font-semibold text-secondary-foreground hover:bg-secondary/80 transition"
                  >
                    Profili Güncelle
                  </button>
                </div>
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground flex items-center gap-1.5 mt-1">
                          <MapPin className="size-3.5" /> {item.location} — {item.department}
                        </p>
                      </div>
                    </div>
                  ))}
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
            <div className="w-full max-w-xl rounded-2xl border border-border bg-card p-6 md:p-8 shadow-xl max-h-[90vh] overflow-y-auto">
              ...
              {formSuccess && (
                <div className="mb-4 p-3 rounded-xl bg-emerald-500/10 text-emerald-600 text-xs font-medium">
                  {formSuccess}
                </div>
              )}
